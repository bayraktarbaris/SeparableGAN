...
class SeparableConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding):
        ...
        self.pointwise = nn.Conv2d(in_channels, out_channels, kernel_size,
                                   stride=stride)  # Normal convolution with 1*1*in_channels kernels

    def forward(self, x):
        return self.pointwise(self.depthwise(x))


class SeparableResBlockGenerator(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1):
        super(SeparableResBlockGenerator, self).__init__()

        self.conv1 = SeparableConvBlock(in_channels, out_channels, 3, 1, padding=1)
        self.conv2 = SeparableConvBlock(out_channels, out_channels, 3, 1, padding=1)
        nn.init.xavier_uniform(self.conv1.weight.data, 1.)
        nn.init.xavier_uniform(self.conv2.weight.data, 1.)
...
class SeparableResBlockDiscriminator(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1):
        super(SeparableResBlockDiscriminator, self).__init__()
        ...
        self.bypass = nn.Sequential()
        if stride != 1:
            self.bypass_conv = SeparableConvBlock(in_channels, out_channels, 1, 1, padding=0)
            nn.init.xavier_uniform(self.bypass_conv.weight.data, np.sqrt(2))
...
# special ResBlock just for the first layer of the discriminator
class SeparableFirstResBlockDiscriminator(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1):
        super(SeparableFirstResBlockDiscriminator, self).__init__()
...
class SeparableGenerator(nn.Module):
    def __init__(self, z_dim):
        ...
        self.dense = nn.Linear(self.z_dim, 4 * 4 * GEN_SIZE)
        self.final = SeparableConvBlock(GEN_SIZE, channels, 3, stride=1, padding=1)
        nn.init.xavier_uniform(self.dense.weight.data, 1.)
        nn.init.xavier_uniform(self.final.weight.data, 1.)

        self.model = nn.Sequential(
            SeparableResBlockGenerator(GEN_SIZE, GEN_SIZE, stride=2),
            SeparableResBlockGenerator(GEN_SIZE, GEN_SIZE, stride=2),
            SeparableResBlockGenerator(GEN_SIZE, GEN_SIZE, stride=2),
            nn.BatchNorm2d(GEN_SIZE),
            nn.ReLU(),
