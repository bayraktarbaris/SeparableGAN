    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        if batch_idx % 1000 == 0:
            ...
            if incepScore > bestInceptionScore:  # Save the models as best generator and discriminator
                ...
                torch.save(discriminator.state_dict(), os.path.join(args.checkpoint_dir, 'disc_{}'.format(
    ...
    if args.model == "resnet":
        if args.cuda_avail == "True":
            ...
            discriminator.load_state_dict(
                torch.load('checkpoints/disc_best' + str(args.experimentNo), map_location=lambda storage, loc: storage))
            generator.load_state_dict(
                torch.load('checkpoints/gen_best' + str(args.experimentNo), map_location=lambda storage, loc: storage))
...
# pretrained=False we will train our networks
    ...
    if args.model == 'resnet':
        ...
        generator = model.TSepConvGenerator(Z_dim, args.batch_size).cuda()
